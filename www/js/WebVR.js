...
Autodesk.Viewing.Extensions.WebVR.StereoRenderContext = function(_vrDisplay, camera, HUD, toolConfig) {
    ...
    this.init = function(renderer, width, height) {
        ...
        toolConfig.webVR_orbitModel = true;
    };
    ...
    this.modeFusion = function(pose) {
        ...
        quat.multiply(new THREE.Quaternion().fromArray(pose.orientation));
        position.applyQuaternion(quat).multiplyScalar(window.distanceToTarget/2);
        camera.position.copy(position.add(camera.pivot));
        camera.lookAt(camera.target);
        camera.up = camera.worldup.clone();
    };
...
Autodesk.Viewing.Extensions.WebVR.VRTool = function(viewer, vrExtension, _vrDisplay) {
    ...
    this.activate = function(name) {
        ...
        $('.adsk-viewing-viewer')[0].style.overflow='visible'; //fix fullscreen bug
        ...
        if (av.isMobileDevice()) {
            ...
            _canvas.addEventListener("touchstart", function(e) {
                if (_isActive && (e.touches.length > 1))
                    self.deactivate();
            });
        }
